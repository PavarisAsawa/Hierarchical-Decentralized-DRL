...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        policy_low: ActorCritic | ActorCriticRecurrent | StudentTeacher | StudentTeacherRecurrent = policy_class(
            num_obs_low, num_obs_low, 8, **policy_cfg_copy
        ).to(self.device)
        self.policies_low = policy_low
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    # Sample actions
                    actions = self.low_policies(obs_low)
    ...
    def load_low_level(self, path: str, device=None):
        loaded_dict = torch.load(path, weights_only=False)
        self.alg_low.policy.load_state_dict(loaded_dict["model_state_dict"])
        self.alg_low.policy.eval()
        if device is not None:
            self.alg_low.policy.to(device)
        self.low_policies = self.alg_low.policy.act_inference
