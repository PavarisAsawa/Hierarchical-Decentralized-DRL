...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        obs = self.env.reset()
        dummy_action = torch.zeros((env.num_envs, env.num_actions)).to(self.device)
        obs, _, _, infos = self.env.step(dummy_action)
        extras = infos
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        # obs, extras = self.env.get_observations()
        obs = self.env.reset()
        dummy_action = torch.zeros((self.env.num_envs, self.env.num_actions)).to(self.device)
        obs, _, _, infos = self.env.step(dummy_action)
        extras = infos
