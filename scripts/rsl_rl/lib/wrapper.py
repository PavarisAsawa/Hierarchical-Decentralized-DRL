...
class RslRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv | DirectRLEnv, clip_actions: float | None = None):
        ...
        if not isinstance(env.unwrapped, ManagerBasedRLEnv) and not isinstance(env.unwrapped, DirectRLEnv) and not isinstance(env.unwrapped, DirectMARLEnv):
            raise ValueError(
        ...
        init_obs, _ = self.unwrapped.reset()
        self.agent_names = list(init_obs.keys())
        self.single_action_space = {agent: env.action_space(agent) for agent in self.agent_names}
        self.num_actions = {
            agent: self.single_action_space[agent].shape[0] for agent in self.agent_names
        }
        self.num_obs = {
            agent: init_obs[agent].shape[1] for agent in self.agent_names
        }
    ...
    def get_observations(self, dict) -> tuple[dict, dict]:
    ...
    def step(self, actions: dict) -> tuple[dict, dict, dict, dict]:
        ...
        if self.clip_actions is not None:
            actions = {
                k: torch.clamp(v, -self.clip_actions, self.clip_actions)
    ...
    def _modify_action_space(self):
        ...
        for agent in self.agent_names:
            dim = self.num_actions[agent]
            self.env.unwrapped.single_action_space[agent] = gym.spaces.Box(
                low=-self.clip_actions, high=self.clip_actions, shape=(dim,)
            )
            self.env.unwrapped.action_space[agent] = gym.spaces.Box(
                low=-self.clip_actions, high=self.clip_actions, shape=(dim,)
            )