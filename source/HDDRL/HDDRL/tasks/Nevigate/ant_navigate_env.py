...
class AntNavigateCfg(DirectRLEnvCfg):
    ...
    goal_pos_visualizer_cfg: VisualizationMarkersCfg = GREEN_ARROW_X_MARKER_CFG.replace(prim_path="/Visuals/Command/position_goal")
...
class AntNavigateEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        pos_env = self.pos_env - self.robot.data.default_root_state[:, :2]
        distance = pos_env - self._commands
        observations =  torch.cat(
                (
                    # High Level Only :)
                    # pos_env ,           # inx 0-1
                    self.heading.unsqueeze(1) ,      # inx 2
                    distance,
                    # self._commands ,    # 3-4
                    # Low level
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:        
        ...
        check_des = torch.norm(self.pos_env - self.robot.data.default_root_state[:, :2]- self._commands)
        truncate = self.episode_length_buf >= self.max_episode_length - 1
        terminate = torch.logical_or(torch.logical_or(self.torso_position[:, 2] < self.cfg.termination_height , self.torso_position[:, 2] > self.cfg.termination_height_up) , check_des < 0.1)
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        # Init marker
            ...
            self.out_vel_visualizer = VisualizationMarkers(self.cfg.current_vel_visualizer_cfg)
    ...
    def _debug_vis_callback(self, event):
        ...
        # Green
        target_pos_w = self.scene.env_origins[:,:3].clone()
        target_pos_w[:, :2] += self._commands
        target_pos_w[:, 2] += 0.5
        # print(base_pos_w, target_pos_w)
        # Blue
        vel_arrow_scale, vel_arrow_quat = self._resolve_xy_velocity_to_arrow(self.robot.data.root_lin_vel_b[:, :2])
        
        # # display markers
        # self.goal_vel_visualizer.visualize(base_pos_w, vel_des_arrow_quat, vel_des_arrow_scale)
        self.out_vel_visualizer.visualize(base_pos_w, vel_arrow_quat, vel_arrow_scale)
        self.goal_pos_visualizer.visualize(target_pos_w, vel_arrow_quat, torch.ones(self.num_envs, 3, device=self.device)*0.5)
        # print((torch.ones(self.num_envs, 3, device=self.device)*0.5).shape)
